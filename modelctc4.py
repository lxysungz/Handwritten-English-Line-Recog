...
def cnn(x, kernel_size, num_neurons, num_conv1, num_conv2, num_conv3, num_conv4, num_conv5,keep_prob):
    """
       Args:
          x: input image (img_height, img_width)
    """
    # VARIABLES
...
def ctc(inputs, labels, seq_len, num_classes, learning_rate=0.01):
    ...
    decoded, log_prob = tf.nn.ctc_beam_search_decoder(inputs, seq_len,merge_repeated=False)
...
def model(config):
    """
        5 layers CNN->bidirectional basic LSTM-> 2 layers Fully Connect-> CTC with beam search decode
        Tensor Shape 
        model       Input Shape                                             Output Shape
        cnn         (?, img_height, img_width)                              (?, img_height/(2**5), img_width/(2**5), num_conv5)
        reshape_1   (?, img_height/(2**5), img_width/(2**5), num_conv5)     (?, img_width/(2**5), num_conv5*img_height/(2**5))
        lstm        (?, img_width/(2**5), num_conv5*img_height/(2**5))      (?, img_width/(2**5), 2*num_neurons)
        reshape_2   (?, img_width/(2**5), 2*num_neurons)                    (?*img_width/(2**5), 2*num_neurons)
        logits      (?*img_width/(2**5), 2*num_neurons)                     (?, num_classes)
        reshape_3   (?, num_classes)                                        (img_width/(2**5), ?, num_classes)
        CTC         (img_width/(2**5), ?, num_classes)                      decoded[0] is a sparse tensor
    """ 
    ...
    with graph.as_default():
       ...
       # Normalization inputs
        ...
        # reshape_1
        # reshape cnn_output with shape (batch_size, rows, columns, channels) to fit 
...
def validation(val_dir, batch_size, ctc_input_len, word_len, level, X, Y, keep_prob, seq_len, session, merged, cost, ler, wer, transcriptionFile):

    """
        
        Args:

          - val_dir: validation images path
          - ctc_input_len: input length of ctc (Int)
          - batch_size:  (Int)
          - word_len: max length of validation word
          - X: Placeholder of input (placeholder)
          - Y: Placeholder of output (placeholder)
          ...
          - merged: Tensor for  merged
          - cost: Tensor for CTC cost 
          - ler: Tensor for ler (letter error rate)
          - wer: Tensor for wer (word error rate

        reuturn:

          - val_tuple: Result of the validation. (Tuple: {'summary','cost','LER', 'WER'})
    """
 
    val_set = util3.dataset(val_dir, batch_size, ctc_input_len, word_len,0, transcriptionFile)
    total_val_cost = 0
    total_val_ler = 0
    total_val_wer = 0
    cont = 1
    ...
    while cont > 0:
        ...
        if len(val_inputs)>0:
            ...
            count += 1
            # print("val step: ", count, "cost: ", val_cost, "ler: ", val_ler)
        else:
            cont = 0
    tf.summary.scalar('cost', total_val_cost/count)
    tf.summary.scalar('ler', total_val_ler/count)
...
def main(_):
    ...
    DEBUG = 1
    SUMMARY_DIR = os.path.curdir + '/summary4/'
    TRAIN_DIR = os.path.curdir+'/LinesOKImages/'
    VAL_DIR = os.path.curdir+'/testImgPad/'
    CHECKPOINT_DIR = os.path.curdir + '/checkpoint4/'
    ...
    # therefor the input to CTC is (time_step=math.ceil(IMG_WIDTH/(2**5)), batch_size, num_classes*IMG_HEIGHT/2**5), 
    # tf.nn.ctc_loss requires ctc_input_len <= math.ceil(IMG_WIDTH/(2**5)/2)
    # with 64X3200 image, time_step=100
    ctc_input_len = 95
    WORD_LEN = 95

    # 自定义参数词典，大多数是模型需要的超级参数
    # #0-73 表示IAM数据集出现的字母和符号字符，74表示NULL，
    # 字母有46个类别，小写和大写类别都归于同一类别字母有：c,o,s,v,w,z
    # 包括的符号字符有 (space)	!	"	#	&	'	(	)	*	+	,	-	.	/	:	;	?	`
    # 
    params = { 
    ...
    with tf.Session(graph=graph) as sess:
        ...
        train_dataset = util3.dataset(TRAIN_DIR, BATCH_SIZE, ctc_input_len, WORD_LEN, 1, TRANSCRIPTIONFILE)
        start_time = time.time()
        for curr_step in range(TRAIN_STEPS):
            
            train_inputs, train_labels, original, train_seq_len = train_dataset.extract_data_batch()                             
            train_feed = {X: train_inputs, Y: train_labels, keep_prob: KEEP_PROB, seq_len: train_seq_len}        
            ...
            if curr_step % PRINT_STEPS == 0:
                ...
                print("Step:", curr_step+global_step, "Train cost:", train_cost, "Train ler:", train_ler, "Duration(s): ", elapse_time)
                print("---------------------------------------------------------------------")
                start_time = time.time()
            if curr_step>0 and curr_step % VAL_STEPS == 0:
                val_start_time = time.time()
                summary, val_cost, val_ler, val_wer = validation(VAL_DIR, BATCH_SIZE, ctc_input_len, 
                                    WORD_LEN, 1, X, Y, keep_prob, seq_len, sess, merged, cost, ler, wer, TRANSCRIPTIONFILE)
                test_writer.add_summary(summary, curr_step+global_step) #here only add cost and ler of the last batch of validation to test writer
                elapse_val_time = time.time() - val_start_time
                print("---------------------------------------------------------------------")
                print("Step:", curr_step+global_step, "Val cost:", val_cost, "Val ler:", val_ler, "Val wer: ", val_wer, "Duration(s): ", elapse_val_time)
