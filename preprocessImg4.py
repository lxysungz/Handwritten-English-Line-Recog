...
def normalizeImg(filename, img,stdRow, stdCol):
    ...
    if len(img.shape)>2:
        #彩色图片
        gray = blackwhiteImg(img,1)
        #plt.imshow(grey)
    elif isBinaryImg(img):
        gray = img
    else:
        #灰度图片
        gray = blackwhiteImg(img, 0)
        #plt.imshow(grey)
    ...
    ratio = min(rowRatio, colRatio)
    gray = cv2.resize(gray, None, fx=ratio, fy=ratio, interpolation=cv2.INTER_CUBIC) 
 
    ret, gray = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    #print("after: ", gray[int(gray.shape[0]/2)][int(gray.shape[1]/4):int(gray.shape[1]*3/4)])
    rows, cols = gray.shape
    #print(gray.shape)
    #rowsPadding = (math.ceil((300-rows)/2),math.floor((300-rows)/2))
    if (stdCol>cols):
        colsPadding = (math.ceil((stdCol-cols)/2),math.floor((stdCol-cols)/2))
        gray = np.lib.pad(gray,((0,0),colsPadding),'constant')
    elif (stdRow>rows):
        rowsPadding = (math.ceil((stdRow-rows)/2),math.floor((stdRow-rows)/2))
        gray = np.lib.pad(gray,(rowsPadding,(0,0)),'constant')    
...
def splitLines(filename, path, stdRow, stdCol):
    grey = cv2.imread(filename)    
    grey = blackwhiteImg(grey,1)
    ...
    lineCount = 0
    for lineImg in imgOfLines:
        img = normalizeImg(filename=None, img=lineImg, stdRow=stdRow, stdCol=stdCol)
        cv2.imwrite(path+'/lines/'+'Line'+str(lineCount)+'.jpg',img)
        lineCount = lineCount + 1
...
if __name__ == '__main__': 
    ...
    stdCol = 3200
    if len(sys.argv) == 1:
        print("Missing image file to process: Python preprocessImg.py filename_image path_store_processed_images image_height image_width")
        
    elif len(sys.argv) == 2:
        print("Start to process image:" + str(sys.argv[1]))
        file = str(sys.argv[1])
        splitLines(str(sys.argv[1]), path, stdRow, stdCol)
    elif len(sys.argv) == 3:
        file = str(sys.argv[1])
        path = str(sys.argv[2])
        splitLines(str(sys.argv[1]), path, stdRow, stdCol)
        ...
        file = str(sys.argv[1])
        path = str(sys.argv[2])
        stdRow = int(sys.argv[3])
        splitLines(str(sys.argv[1]), path, stdRow, stdCol)
    elif len(sys.argv) == 5:
        file = str(sys.argv[1])
        path = str(sys.argv[2])
        stdRow = int(sys.argv[3])
        stdCol = int(sys.argv[4])
        splitLines(str(sys.argv[1]), path, stdRow, stdCol)
    else:
        print("Too many arguments. please enter >>python preprocessImg.py filename_image path_store_processed_images image_height image_width")